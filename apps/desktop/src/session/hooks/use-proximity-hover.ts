...
export function useProximityHover<T extends HTMLElement>(
  ...
  const measureItems = useCallback(() => {
    ...
    itemsRef.current.forEach((element, index) => {
      ...
      // unaffected by CSS transforms (e.g. a scale animation on a parent
      // motion.div). offsetTop/offsetLeft are layout values relative to the
      // offsetParent, matching the coordinate space used by
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      rafIdRef.current = requestAnimationFrame(() => {
        ...
        // scale factor so layout coords map into the same visual viewport
