...
// Edge spring for the selected-bg merge/split: moderate, no bounce so
// converging edges meet exactly instead of overshooting. On a merge the inner
// corners trail by `cornerDelay`, staying rounded until the halves meet.
...
// framer skips that callback when an animation's target equals its current
// value (which spam-toggling produces), which would otherwise strand a half.
// The buffer biases late, by which point the halves have met/parted.
...
export interface SelBlock extends Rect {
  ...
  // instead of snapping when continuity is lost (fast toggling) or the block
  // is inherently new (a split's lower half). Continuous blocks ignore it.
  enterFrom?: {
    top: number;
    height: number;
...
// to the bridging row's midpoint (facing corners straightening to sharp), then
// swap to one block with no visible motion — instead of the surviving block
...
export function useMergeSplitBlocks(
  ...
  // bridge row was toggled again mid-flight). Keyed on runsSig: it encodes
  // the runs' identity, so the effect re-runs exactly when the runs change.
  useLayoutEffect(() => {
    ...
    setBoundaries((active) => [
      ...active.filter((b) =>
        b.kind === "merge"
          ...
          : cur.some(
              (c) => c.id === b.survivorId && c.end === b.gapIndex - 1,
            ) &&
  ...
  for (const b of boundaries) {
    ...
    if (b.kind === "merge") {
      ...
      // inner corners straightening to sharp. Slightly trail lower merges
      // while keeping a baseline and small cap.
  ...
  // created in a layout effect that runs *after* this render, so on the very
...
// Renders the selected-background blocks produced by useMergeSplitBlocks —
// one per run, or two abutting halves mid merge/split. `dimmed` drops the
// opacity to 0.8 (when the user is hovering a non-selected row) to match the
// standalone hover indicator; a block's own `opacity` override (e.g. the
// commit ghost) always wins. Corners are driven numerically so merge/split
// can straighten and re-round individual corners.
      ...
      {blocks.map((b) => {
        ...
        return (
          <motion.div
            ...
            exit={{
              opacity: 0,
              transition: { duration: b.exitInstant ? 0 : 0.12 },
            }}
            transition={
              b.instant
                ? { duration: 0 }
