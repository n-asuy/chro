...
/**
 * Agent brand marks, keyed by the bare agent kind stored on a task's
 * `last_executor` (see `TaskRecord::set_last_executor` on the backend, which
 * writes the `BaseCodingAgent` discriminant — "CLAUDE_CODE" / "CODEX").
 *
 * These render the agent that actually ran a session so surfaces like the
 * session tab can show a recognizable logo instead of a generic chat glyph.
 */

/** OpenAI / Codex logomark. Monochrome, inherits the surrounding text color. */
...
/** Claude Code logomark (the genuine bundled sunburst asset, full color). */
