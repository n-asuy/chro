...
The bundled Rust server now uses an explicit CORS allowlist. Defaults cover the desktop renderer in dev (`http://localhost:3400`, `http://127.0.0.1:3400`) and the packaged app on every platform: macOS and Linux load it from `tauri://localhost`, while the Windows webview serves the same assets over `http://tauri.localhost` (`https://tauri.localhost` when the secure scheme is enabled). Add browser extension origins with `CHRO_ALLOWED_ORIGINS`, for example `CHRO_ALLOWED_ORIGINS=chrome-extension://<extension-id>`.
